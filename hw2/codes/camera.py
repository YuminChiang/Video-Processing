...
class Camera():
    def __init__(self, fov, image_width, image_height, frames_per_second):
        # Projection matrix
        F = fov * np.pi / 180 
        ...
        # Compute actual line scanning time and scanned lines in a frame
        # Line scanning time = (line interval) - (horizonal retrace time)
    ...
    def raster_scan(self, delta_time_ms):
        ...
        t0 = self.elapsed_time_ms_scan # 上一次的時間
        t1 = self.elapsed_time_ms_scan + delta_time_ms # 這次的時間
        sec2ms = 1000
        T_frame_ms = sec2ms / self.fps # frame interval
        l = self.scanned_lines # 目前掃了幾條線

        if t1 <= T_frame_ms - self.vertical_retrace_time: # Before vertical retracing 影像正在做 line scanning 或 horizontal re-tracing
           # Determine the elapsed time after horizontally retracing on current line
           t_line = t1 - l * self.line_interval
           if t_line <= self.line_scanning_time: # 還在 line scanning time，找最新的 segment
               # TODO #4: Finish line scanning by implementing the following procedure to replace 'pass' command:
               # 1. Determine scanning segment based on previous raster index, t_line and raster rate
               # 2. Employ raster_scan_one_pixel to execute scanning for each individual pixel
               scanned_pixels = int(t_line / self.pixel_interval) - (self.scan_index % self.w)
               for _ in range (scanned_pixels):
                   self.raster_scan_one_pixel()
           elif t_line <= self.line_interval: # Re-tracing 
               pass
           else:
               # TODO #5: Finish horizontal re-tracing to replace 'pass' command
    ...
    def raster_scan_one_pixel(self):
        ...
        # Let (x[0], x[1]) denote the image coordinate, then scan_index = x[1] * w + x[0].
        x = [0, 0]
