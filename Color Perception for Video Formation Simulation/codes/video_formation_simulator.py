...
delta_time_ms_light = 10 # in milli seconds, lighting 變化時間間隔
...
frame = 0
ms2sec = 0.001
delta_time_ms_frame = 1000 / fps # frame 之間的間隔
while frame < 300:
	...
	# Capture lights in camera
        x = camera.project_to_image_position(light.X) # x 是 proj 的位置
        camera.write_image(x, r, g, b)

	# TODO: Simulate attenuation and gamma corrected color on the display
        display_color = [r, g, b]     # with color correction
        display_color_ori = [r, g, b] # without color correction

        vc = camera.get_output_voltage(display_color) # output voltage of the camera
        gc = camera.get_gamma()                       # camera signal distortion
        vd = display.gamma_correct(vc, gc)            # input voltage of the display

        display_color_ori = display.output_brightness(vc) # nonlinear distortion with respect to brightness
        display_color = display.output_brightness(vd) # final brightness of the display
    ...
    if time_ms >= frame * delta_time_ms_frame: # 寫 video 的時機，以 frame 的間隔來決定，世界真實情況!
